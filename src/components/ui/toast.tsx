...
import { Check, X, Info } from "lucide-react"; // Import Info เพิ่มเติม
            ...
            <div className="fixed top-4 right-4 z-[250] flex flex-col gap-3 pointer-events-none">
                {toasts.map((toast) => {
                    ...
                    return (
                        <div
                            key={toast.id}
                            className={`pointer-events-auto w-80 max-w-[90vw] rounded-2xl shadow-lg border-l-4 ${style.bg} ${style.border} p-4 flex gap-3 items-start animate-in slide-in-from-right-5 duration-300`}
                        >
                            <div className={`mt-0.5 rounded-full p-1.5 ${style.iconBg} ${style.iconColor} flex items-center justify-center`}>
                                {toast.type === "success" ? (
                                    <Check className="h-5 w-5" />
                                ) : toast.type === "error" ? (
                                    <X className="h-5 w-5" />
                                ) : (
                                    /* เปลี่ยนจาก <span>i</span> เป็นไอคอน Info */
